...
def set1(request):
    ...
    return render(request, 'stock/set1.html', {})
# ---
...
def getModel(request,username = None):
    ...
    for i,j in enumerate(all):
        if request.GET.get(j):
            tmp = []
            name.append(str(j))
            value.append(request.GET.get(j))
            tmp.append(float(request.GET.get(j)))
            index[str(j)] = tmp
    cols = xa.columns.values.tolist()
    data = pd.DataFrame(index,columns=cols)
    if request.GET.get('model') == 'rfr':
        estimator = RandomForestRegressor(random_state=0, n_estimators=100)
        estimator.fit(a,b)
        answer = estimator.predict(data)
    elif request.GET.get('model') == 'gbr':
        est = GradientBoostingRegressor(n_estimators=100, learning_rate=0.1,max_depth=1, random_state=0, loss='ls').fit(a, b)
        answer = est.predict(data)
    elif request.GET.get('model') == 'dt':
        clf = tree.DecisionTreeRegressor()
        clf.fit(a,b)
        answer = clf.predict(data)
    elif request.GET.get('model') == 'ann':
        model_mlp = MLPRegressor(solver='lbfgs', hidden_layer_sizes=(20,20,20), random_state=1) 
        model_mlp.fit(a,b)
        answer = model_mlp.predict(data)
    else:
       xgr = XGBRegressor()
       xgr.fit(xa,b)
       answer = xgr.predict(data)
     
    ind_list = ind.objects.all()
    t = get_template('/home/ubuntu/workspace/test/test1/stockrobot/templates/stock/result.html')
    c = template.Context({'answer':answer,'code':code,'ind_list':index})
